...
std::string serve_usage_text(const char* argv0) {
    return std::string("usage: ") + argv0 +
           ...
           "       --request-log-jsonl appends full-precision server/request records readable "
           "by all OS users\n"
           "       --request-log-content-dir atomically persists sensitive model-visible prompts "
           "and final responses as Markdown readable by all OS users (requires "
           "--request-log-jsonl); operators own parent-directory access, retention and rotation\n"
